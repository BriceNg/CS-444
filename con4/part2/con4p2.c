...
void * agent(void * arg){
	while(1) {
		...
		if ( randNum == 0 ) {
			...
			have_match = 1;
			have_paper = 1;
			puts("Agent sells paper and match.");
		...
		//Tobacco and Match
			...
			have_match = 1;
			have_tobacco = 1;
			puts("Agent sells tobacco and match.");
			pthread_cond_signal(&paper);
			pthread_cond_signal(&match);
		}
		...
		//Tobacco and Paper
			...
			have_tobacco = 1;
			have_paper = 1;
			puts("Agent sells paper and tobacco.");
			pthread_cond_signal(&paper);
...
void * pusher_paper(void * arg){
	while(1){
		...
		if(have_match == 1) {
			...
			puts("The tobacco owning smoker buys.");
		...
		if(have_tobacco == 1) {
			...
			agent_job = 0;
			smoker_match_job = 1;
			puts("The match owning smoker buys.");
...
void * pusher_match(void * arg){
	while(1) {
		...
		if(have_tobacco == 1) {
			...
			agent_job = 0;
			smoker_paper_job = 1;
			puts("The paper owning smoker buys.");
...
void * smoker_tobacco(void * arg){
	while(1){
		...
		agent_job = 1;
		puts("Tobacco owning Smoker makes cigarette.");
		pthread_mutex_unlock(&smoker);
		puts("Tobacco owning Smoker is smoking.");
	}
	return 0;
}
...
void * smoker_paper(void * arg){
	while(1){
		...
		smoker_paper_job = 0;
		agent_job = 1;
		puts("Paper owning Smoker makes cigarette.");
		pthread_mutex_unlock(&smoker);
		puts("Paper owning Smoker is smoking.");
	}
	return 0;
}
...
void * smoker_match(void * arg){
	while(1){
		...
		smoker_match_job = 0;
		agent_job = 1;
		puts("Match owning Smoker makes cigarette.");
		pthread_mutex_unlock(&smoker);
		puts("Match owning Smoker is smoking.");
	}
	return 0;
}
